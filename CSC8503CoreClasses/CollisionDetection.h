...
namespace NCL {
	...
	public:
		struct ContactPoint {
			Vector3 localA; // where did the collision occur in the frame of each object!
			Vector3 localB; // where did the collision occur in the frame of each object!
			Vector3 normal;
			float	penetration;
		};
		...
		static bool useSAT(const Vector3 delta, Vector3 plane, const Transform& worldTransformA, const Transform& worldTransformB, const Vector3 halfSizeA, const Vector3 halfSizeB, float& penDistance, Vector3& normal, Vector3& pointA, Vector3& pointB);
