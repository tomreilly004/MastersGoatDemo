	...
	for (auto i = first; i != last; ++i) {
		...
		for (auto j = i + 1; j != last; ++j) {
			...
			if (CollisionDetection::ObjectIntersection(*i, *j, info)) {
				...
				if (info.a->GetName() == "Goat" && info.b->GetName() == "floor") {
					ResolveSpringCollision(*info.a, *info.b, info.point);
					std::cout << "spring";
				} 
				else if (info.b->GetName() == "Goat" && info.a->GetName() == "floor") {
					ResolveSpringCollision(*info.a, *info.b, info.point);
					std::cout << "spring";
				}
				else {
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	float cRestitution = 0.66f * a.GetPhysicsObject()->GetElasticity() * b.GetPhysicsObject()->GetElasticity(); // disperse some kinectic energy
	...
	if (a.frictionOn || b.frictionOn) {
		Vector3 tangent = contactVelocity - (p.normal * Vector3::Dot(contactVelocity, p.normal));
		tangent.Normalise();
		Vector3 inertiaFricA = Vector3::Cross(physA->GetInertiaTensor() * Vector3::Cross(relativeA, tangent), relativeA);
		Vector3 inertiaFricB = Vector3::Cross(physB->GetInertiaTensor() * Vector3::Cross(relativeB, tangent), relativeB);
		float angularEffectFriction = Vector3::Dot(inertiaFricA + inertiaFricB, tangent);
		float frictionCoeff = 0.1;
		float j = (-(frictionCoeff * Vector3::Dot(contactVelocity, tangent)) / (totalMass + angularEffectFriction));
		fullImpulse = tangent * j;
		if (a.frictionOn)physA->ApplyLinearImpulse(-fullImpulse);
		if (b.frictionOn)physB->ApplyLinearImpulse(fullImpulse);
	}

}


// Collisions using Hooke's Law penalty method.
void PhysicsSystem::ResolveSpringCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const
{
	...
	Vector3 springExtensionA = p.localA + (p.normal * p.penetration);
	Vector3 springExtensionB = p.localB - (p.normal * p.penetration);

	float k = 10.0;

	const Vector3 forceOnObjectA = springExtensionA * -k;
	const Vector3 forceOnObjectB = springExtensionB * -k;

	physicsObjectA->ApplyLinearImpulse(forceOnObjectA);
	physicsObjectB->ApplyLinearImpulse(forceOnObjectB);
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	std::vector < GameObject* >::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	float frameLinearDamping = 1.0f - (globalDamping * dt); // DAMPING = 0.4
